	...
	$.fn.speedometer = function( options ){
		...
		var def = {
			...
			thisCss: {
				...
				fontFamily: 'Arial',
				fontWeight: '900',
				background : "url('img/ybp_bj.png') no-repeat 0 5px"
                //backgroundSize:"150px" //无
			},
			digitalCss: {
        ...
        if(def.limit&&def.percentage > def.maximum){
            def.percentage = def.maximum
        }
			...
			$this.jqcanvas ( function ( canvas, width, height ) {
				...
				if (def.animate) {
					...
					def.increment = ( i < def.percentage ) ? 0.2 : -1;
				} else {
